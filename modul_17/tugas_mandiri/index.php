...
<body>
  <div class="wrap">
    ...
    <?php if (!$isAdmin): ?>
      <div class="info-user">
        &#9432; Anda login sebagai <strong>User</strong>. Hanya dapat melihat dan melihat detail data siswa.
      </div>
    <?php endif; ?>
    ...
    <div class="card">
      <table>
        ...
        <tbody>
          ...
          ?>
            <tr>
              <td colspan="10" class="empty">Belum ada data siswa.</td>
            </tr>
          <?php else:
            while ($row = mysqli_fetch_assoc($result)): ?>
              <tr>
                ...
                <td><?= $row["jk"] === "L" ? "L" : "P" ?></td>
                <td>
                  <?php foreach (array_filter(array_map("trim", explode(",", $row["hobi"]))) as $h): ?>
                    <span class="badge badge-hobi"><?= htmlspecialchars($h) ?></span>
                  <?php endforeach; ?>
                </td>
                <td>
                  <?php foreach (array_filter(array_map("trim", explode(",", $row["ekskul"]))) as $e): ?>
                    <span class="badge badge-ekskul"><?= htmlspecialchars($e) ?></span>
                  <?php endforeach; ?>
                </td>
                <td>
                  <div class="aksi-group">
                    <!-- Detail: semua level bisa akses -->
                    <a href="detail.php?nis=<?= urlencode($row["nis"]) ?>" class="button button-info">Detail</a>
                    <?php if ($isAdmin): ?>
                      <a href="edit.php?nis=<?= urlencode($row["nis"]) ?>" class="button button-warning">Edit</a>
                      <a href="hapus.php?nis=<?= urlencode($row["nis"]) ?>"
                        class="button button-danger"
                        onclick="return confirm('Hapus data <?= htmlspecialchars(addslashes($row["nama"])) ?>?')">Hapus</a>
                    <?php endif; ?>
                  </div>
                </td>
              </tr>
